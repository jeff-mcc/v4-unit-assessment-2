...
for (let key in shapes){
    shapes[key] % 2 === 0 ? '' : delete shapes[key];
}
...
for (let i = 0; i<classes.length; i++){
    for (let key in classes[i]){
        classes[i][key] === true ? classes[i][key] = false : '';
    }
}
...
//CODE HERE
  let phone1 = new Phone('pear','P1',64,'green',false);
  
  let phone2 = new Phone('mango','M1',128,'orange',false);
  
  let phone3 = new Phone('grapefruit','G1',32,'yellow',false);
